...
async function getEventAndVerify(
  eventId: string,
  userId: string,
  role: string,
) {
  ...
  if (role !== "super_admin" && event.organizerId.toString() !== userId)
    return null;
  return event;
}
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  await connectDB();
  const event = await getEventAndVerify(
    params.eventId,
    session.user.id,
    session.user.role,
  );
