...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-obsidian-950 flex">
      {/* ─── Sidebar ──────────────────────────────────────── */}
      <aside className="w-60 flex-shrink-0 bg-obsidian-900 border-r border-obsidian-800 flex flex-col sticky top-0 h-screen">
        {/* Logo */}
        <div className="px-6 py-6 border-b border-obsidian-800">
          <Link href="/dashboard" className="font-display text-xl italic text-gold-500">
            FêtEasy
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <li key={item.href}>
                  ...
                  >
                    <Icon className={cn("w-4 h-4 flex-shrink-0", active ? "text-gold-500" : "text-obsidian-600 group-hover:text-obsidian-400")} />
                    <span className="text-[0.52rem] tracking-[0.2em] uppercase font-body font-light">
                      {item.label}
                    </span>
                    {active && <ChevronRight className="w-3 h-3 ml-auto text-gold-700" />}
                  </Link>
                </li>
              );
            })}
        ...
        <div className="px-3 py-4 border-t border-obsidian-800">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-obsidian-800/30 mb-2">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-obsidian-600 hover:text-red-400 hover:bg-red-950/20 transition-all"
          >
