...
export default function DashboardPage() {
  ...
  const totalConfirmed = events.reduce((s, e) => s + (e.stats?.confirmed || 0), 0);
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        ...
        <StatCard label="Confirmés" value={totalConfirmed} icon={CheckCircle} accent />
        <StatCard
          label="Taux de confirmation"
          value={totalGuests ? Math.round((totalConfirmed / totalGuests) * 100) + "%" : "—"}
          icon={Clock}
        />
      </div>
        ...
        <div className="flex flex-col gap-3">
          {events.slice(0, 5).map((event) => (
            <Link
              ...
              className="group bg-obsidian-900 border border-obsidian-800 hover:border-gold-900/50 rounded-xl px-6 py-4 flex items-center gap-6 transition-all duration-200"
            >
              <div className="flex-1 min-w-0">
                <p className="font-body font-light text-obsidian-200 text-sm truncate group-hover:text-white transition-colors">
                  {event.title}
                </p>
                <p className="text-[0.48rem] tracking-widest uppercase text-obsidian-600 mt-0.5">
                  {formatEventDate(event.date)} · {event.venue}
                </p>
              </div>
              <div className="flex items-center gap-4 text-right flex-shrink-0">
                <div>
                  <p className="font-display text-xl text-gold-500">{event.stats?.confirmed || 0}</p>
                  <p className="text-[0.42rem] tracking-widest uppercase text-obsidian-700">confirmés</p>
                </div>
                <StatusBadge status={event.isPublished ? "confirmed" : "pending"} />
