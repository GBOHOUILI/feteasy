...
export default function InvitePage() {
    ...
    fetch(`/api/events/by-slug/${slug}`)
      ...
      .then((d) => {
        if (d.success) {
          ...
          setEventError("Lien d'invitation invalide ou expiré.");
        }
      })
  ...
  useEffect(() => {
    ...
    import("qrcode").then((QRCode) => {
      QRCode.toDataURL(ticket.guest.code, { width: 160, margin: 2 })
        .then(setQrDataUrl)
        .catch(console.error);
    });
  }, [ticket]);
  ...
  async function downloadImage() {
    ...
    toast("Génération de l'image…", { icon: "⏳" });
    try {
      ...
      a.download = `ticket-${ticket.event.title.replace(/\s+/g, "-").toLowerCase()}-${ticket.guest.code}.png`;
      a.click();
  ...
  // ── Téléchargement PDF (ticket taille carte, centré) ────
  async function downloadTicket() {
    ...
    try {
      ...
      // Ticket compact — environ 85mm de large (format carte), centré dans la page
      const imgW = 85;
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-up">
        ...
        {ticket && (
          <div className="animate-fade-up">
            ...
            {/* ── Ticket (capturé par html2canvas) ── */}
            <div
              ref={ticketRef}
              className="bg-obsidian-900 border border-gold-800/30 rounded-2xl p-8 relative overflow-hidden"
            >
              {/* Ligne dorée en haut */}
              <div className="absolute top-0 left-[15%] right-[15%] h-px bg-gradient-to-r from-transparent via-gold-500 to-transparent" />

              {/* En-tête */}
              <p className="text-[0.48rem] tracking-[0.4em] uppercase text-gold-500 text-center mb-1 font-body font-light">
                ✦ &nbsp; Ticket numérique &nbsp; ✦
              </p>
              <h2 className="font-display font-light text-lg italic text-obsidian-400 text-center mb-0.5">
                Bienvenue,
              </h2>
              ...
              {/* Champs événement */}
              <div className="grid grid-cols-2 gap-x-6 gap-y-5 mb-2">
                {[
                  ...
                  <div key={k}>
                    {/* ↓ text-gold-500 au lieu de text-gold-800 — bien plus lisible */}
                    <p className="text-[0.44rem] tracking-[0.32em] uppercase text-gold-500 mb-1 font-body font-light">
                      {k}
                    </p>
                    ...
                    {sub && (
                      <p className="text-[0.46rem] tracking-wide text-obsidian-500 mt-0.5 font-body font-light">
                        {sub}
                      </p>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-0 my-5 -mx-8">
                <div className="w-3 h-3 rounded-full bg-obsidian-950 border border-gold-900/30 shrink-0" />
                <div className="flex-1 border-t border-dashed border-gold-900/25" />
                <div className="w-3 h-3 rounded-full bg-obsidian-950 border border-gold-900/30 shrink-0" />
              </div>

              {/* Code d'accès */}
              <div className="bg-obsidian-950/60 border border-obsidian-800 rounded-lg px-4 py-3 text-center mb-5">
                <p className="text-[0.44rem] tracking-[0.3em] uppercase text-gold-600 mb-1 font-body font-light">
                  Code d&apos;accès
                </p>
              ...
              <p className="text-center text-[0.44rem] tracking-[0.28em] uppercase text-obsidian-600 font-body font-light">
